...
from . import FmdlFile, Ftex, IO, PesSkeletonData, CompatibilityLayer
		...
		for update in depsgraph.updates:
			if isinstance(update.id, bpy.types.Mesh):
				if update.is_updated_geometry or update.is_updated_transform:
					...
					meshObjectList.append(update.id)
...
def showExportSummary(area, objectName):
	...
	if textName in bpy.data.texts:
		if area is None:
			print(bpy.data.texts[textName])
			fname = f"summary-{objectName}.txt";
			with open(fname, 'a+') as f:
				f.write(bpy.data.texts[textName])
			os.startfile(fname)
		else:
...
class FMDL_Scene_Export_Object_Summary(bpy.types.Operator):
	...
	def execute(self, context):
		...
		showExportSummary(None, self.objectName)
		return {'FINISHED'}
...
def createPesSkeleton(context, skeletonType):
	...
	shim.setActiveObject(blenderArmatureObject)
	...
	def execute(self, context):
		...
		for objectID in removeList:
			...
			while len(bpy.data.objects[objectID].users_scene) > 0:
				shim.unlinkObjectFromScene(objectID)
...
class FMDL_Material_Panel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		listButtonColumn.operator("fmdl.material_parameter_add", icon = 'ZOOM_IN', text = "")
		listButtonColumn.operator("fmdl.material_parameter_remove", icon = 'ZOOM_OUT', text = "")
...
def register():
	...
	shim.AppendSceneUpdatePostEvent(FMDL_Util_TrackChanges)

def unregister():
	shim = CompatibilityLayer.CompatibilityLayer()
	shim.RemoveSceneUpdatePostEvent(FMDL_Util_TrackChanges)
